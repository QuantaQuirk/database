...
use QuantaQuirk\Database\SQLiteDatabaseDoesNotExistException;
use QuantaQuirk\Database\SqlServerConnection;
use PDOException;
use Throwable;
...
class MigrateCommand extends BaseCommand implements Isolatable
{
    ...
    {
        if ($this->quantaquirk['config']->get("database.connections.{$connection->getName()}.database") !== $connection->getDatabaseName()) {
            return false;
        }
        ...
        try {
            $this->quantaquirk['config']->set("database.connections.{$connection->getName()}.database", null);

            $this->quantaquirk['db']->purge();
            ...
            $this->quantaquirk['config']->set("database.connections.{$connection->getName()}.database", $connection->getDatabaseName());
        }
    }
