...
namespace QuantaQuirk\Database\Query\Grammars;
...
class SqlServerGrammar extends Grammar
{
     ...
     *
     * @param  \QuantaQuirk\Database\Query\Builder  $query
     * @param  \QuantaQuirk\Database\Query\IndexHint  $indexHint
     * @return string
     */
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $sql .= 'using (values '.$parameters.') '.$this->wrapTable('quantaquirk_source').' ('.$columns.') ';

        $on = collect($uniqueBy)->map(function ($column) use ($query) {
            return $this->wrap('quantaquirk_source.'.$column).' = '.$this->wrap($query->from.'.'.$column);
        })->implode(' and ');
        ...
        if ($update) {
            $update = collect($update)->map(function ($value, $key) {
                return is_numeric($key)
                    ? $this->wrap($value).' = '.$this->wrap('quantaquirk_source.'.$value)
