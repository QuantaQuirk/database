...
class MySqlSchemaState extends SchemaState
{
    ...
    public function dump(Connection $connection, $path)
    {
        $this->executeDumpProcess($this->makeProcess(
            $this->baseDumpCommand().' --routines --result-file="${:QUANTAQUIRK_LOAD_PATH}" --no-data'
    ...
    {
        $command = 'mysql '.$this->connectionString().' --database="${:QUANTAQUIRK_LOAD_DATABASE}" < "${:QUANTAQUIRK_LOAD_PATH}"';
    ...
    protected function baseDumpCommand()
    {
        ...
        return $command.' "${:QUANTAQUIRK_LOAD_DATABASE}"';
    }
    ...
    {
        $value = ' --user="${:QUANTAQUIRK_LOAD_USER}" --password="${:QUANTAQUIRK_LOAD_PASSWORD}"';

        $config = $this->connection->getConfig();

        $value .= $config['unix_socket'] ?? false
                        ? ' --socket="${:QUANTAQUIRK_LOAD_SOCKET}"'
                        : ' --host="${:QUANTAQUIRK_LOAD_HOST}" --port="${:QUANTAQUIRK_LOAD_PORT}"';

        if (isset($config['options'][\PDO::MYSQL_ATTR_SSL_CA])) {
            $value .= ' --ssl-ca="${:QUANTAQUIRK_LOAD_SSL_CA}"';
        }
    ...
    protected function baseVariables(array $config)
    {
        ...
        return [
            'QUANTAQUIRK_LOAD_SOCKET' => $config['unix_socket'] ?? '',
            ...
            'QUANTAQUIRK_LOAD_PASSWORD' => $config['password'] ?? '',
            'QUANTAQUIRK_LOAD_DATABASE' => $config['database'],
            'QUANTAQUIRK_LOAD_SSL_CA' => $config['options'][\PDO::MYSQL_ATTR_SSL_CA] ?? '',
        ];
    }
